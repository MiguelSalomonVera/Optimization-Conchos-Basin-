# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,2: Delicias-Bajo Conchos using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
region = {"Delicias": {"volumen_mm3": 976309.62, "lamina_reportada": 1.44},
          ...
          "Florido": {"volumen_mm3": 64095.84, "lamina_reportada": 1.74}} # en m 
...
distritos = ["Delicias","BConchos","Florido"]
models = {}
...
restricciones_conjuntas={"fodder_production_min": {"Delicias": 500000, "BConchos":20000,"Florido":20000 }}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "BConchos": ["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]# o "BConchos" o "Florido", son iguales
escenarios_coalicion = [(d, d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_bc, prop_Flor in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_bc, 2), round(prop_Flor, 2) )
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% , {prop_bc*100:.0f}% y {prop_Flor*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_bc if d == "BConchos" else prop_Flor, 2)]for d in coalicion for c in data[d]})
    ...
    step4.volumen_disponible_base = Param(step4.distritos, initialize={"Delicias": region["Delicias"]["volumen_mm3"] * 1e3 * prop_del,"BConchos": region["BConchos"]["volumen_mm3"] * 1e3 * prop_bc,
                                          "Florido": region["Florido"]["volumen_mm3"] * 1e3 * prop_Flor})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = ((eficiencia_transferencia * step4.transfiere_bc_a_del)+(eficiencia_transferencia* step4.transfiere_Flor_a_del)) - ((step4.transfiere_del_a_bc*.8)+(step4.transfiere_del_a_Flor*.8))
        elif d == "BConchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_bc)+(eficiencia_transferencia1* step4.transfiere_Flor_a_bc)) - ((step4.transfiere_bc_a_del*.8)+(step4.transfiere_bc_a_Flor*.8))
        elif d == "Florido":
           ajuste = ((eficiencia_transferencia1 * step4.transfiere_del_a_Flor)+(eficiencia_transferencia1* step4.transfiere_bc_a_Flor)) - ((step4.transfiere_Flor_a_del*.8)+(step4.transfiere_Flor_a_bc*.8))
        else:
           ajuste = 0
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del_bc,transferencia_opt_del_Flor,transferencia_opt_BC_del,transferencia_opt_BC_Flor,transferencia_opt_Flor_del,transferencia_opt_Flor_bc)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, BConchos={clave[1]*100:.0f}%,Florido={clave[2]*100:.0f}%")
    ...
    print("Transferencia BConchos → Delicias: {:,.2f} m³".format(beneficio_info[3]))
    print("Transferencia BConchos → Florido: {:,.2f} m³".format(beneficio_info[4]))
    print("Transferencia Florido → Delicias: {:,.2f} m³".format(beneficio_info[5]))
    print("Transferencia Florido → BConchos: {:,.2f} m³".format(beneficio_info[6]))
