# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,2,4: Delicias-Florido-AC using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
distritos = ["Delicias","Florido","Aconchos"]
models = {}
...
coalicion = ["Delicias","Florido","Aconchos"]
...
restricciones_por_distrito = {
    ...
    "Aconchos": {"nut_area_min": 7634*0.55}}
restricciones_conjuntas={"fodder_production_min": {"Delicias": 500000, "Florido":20000,"Aconchos":100 }}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "Florido": ["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"],
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]  # o "Florido" o "AC", son iguales
escenarios_coalicion = [(d, d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_FLOR, prop_AC in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_FLOR, 2), round(prop_AC, 2) )
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% , {prop_FLOR*100:.0f}% y {prop_AC*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_FLOR if d == "Florido" else prop_AC, 2)]for d in coalicion for c in data[d]})
    ...
    step4.volumen_disponible_base = Param(step4.distritos, initialize={"Delicias": region["Delicias"]["volumen_mm3"] * 1e3 * prop_del,"Florido": region["Florido"]["volumen_mm3"] * 1e3 * prop_FLOR,
                                          "Aconchos": region["Aconchos"]["volumen_mm3"] * 1e3 * prop_AC})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = ((eficiencia_transferencia * step4.transfiere_FLOR_a_del)+(eficiencia_transferencia* step4.transfiere_AC_a_del)) - ((step4.transfiere_del_a_FLOR*.8)+(step4.transfiere_del_a_AC*.8))
        elif d == "Florido":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_FLOR)+(eficiencia_transferencia* step4.transfiere_AC_a_FLOR)) - ((step4.transfiere_FLOR_a_del*.8)+(step4.transfiere_FLOR_a_AC*.8))
        elif d == "Aconchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_AC)+(eficiencia_transferencia* step4.transfiere_FLOR_a_AC)) - ((step4.transfiere_AC_a_del*.8)+(step4.transfiere_AC_a_FLOR*.8))
        else:
           ajuste = 0
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del_FLOR,transferencia_opt_del_AC,transferencia_opt_FLOR_del,transferencia_opt_FLOR_AC,transferencia_opt_AC_del,transferencia_opt_AC_FLOR)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, Florido={clave[1]*100:.0f}%,Aconchos={clave[2]*100:.0f}%")
    print("Beneficio total conjunto: ${:,.2f}".format(beneficio_info[0]))
    print("Transferencia Delicias → Florido: {:,.2f} m³".format(beneficio_info[1]))
    print("Transferencia Delicias → Aconchos: {:,.2f} m³".format(beneficio_info[2]))
    print("Transferencia Florido → Delicias: {:,.2f} m³".format(beneficio_info[3]))
    print("Transferencia Florido → Aconchos: {:,.2f} m³".format(beneficio_info[4]))
    print("Transferencia Aconchos → Delicias: {:,.2f} m³".format(beneficio_info[5]))
    print("Transferencia Aconchos → Florido: {:,.2f} m³".format(beneficio_info[6]))
