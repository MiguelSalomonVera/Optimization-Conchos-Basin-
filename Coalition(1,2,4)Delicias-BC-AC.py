# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,2,4: Delicias-Bajo Conchos-AC using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
distritos = ["Delicias","BConchos","Aconchos"]
models = {}
...
coalicion = ["Delicias","BConchos","Aconchos"]
...
restricciones_conjuntas={"fodder_production_min": {"Delicias": 500000, "BConchos":20000,"Aconchos":100 }}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "BConchos": ["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]  # o "BConchos" o "AC", son iguales
escenarios_coalicion = [(d, d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_bc, prop_AC in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_bc, 2), round(prop_AC, 2) )
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% , {prop_bc*100:.0f}% y {prop_AC*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_bc if d == "BConchos" else prop_AC, 2)]for d in coalicion for c in data[d]})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = ((eficiencia_transferencia * step4.transfiere_bc_a_del)+(eficiencia_transferencia* step4.transfiere_AC_a_del)) - ((step4.transfiere_del_a_bc*.8)+(step4.transfiere_del_a_AC*.8))
        elif d == "BConchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_bc)+(eficiencia_transferencia* step4.transfiere_AC_a_bc)) - ((step4.transfiere_bc_a_del*.8)+(step4.transfiere_bc_a_AC*.8))
        elif d == "Aconchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_AC)+(eficiencia_transferencia* step4.transfiere_bc_a_AC)) - ((step4.transfiere_AC_a_del*.8)+(step4.transfiere_AC_a_bc*.8))
        else:
           ajuste = 0
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del_bc,transferencia_opt_del_AC,transferencia_opt_BC_del,transferencia_opt_BC_AC,transferencia_opt_AC_del,transferencia_opt_AC_bc)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, BConchos={clave[1]*100:.0f}%,Aconchos={clave[2]*100:.0f}%")
    ...
    print("Transferencia BConchos → Aconchos: {:,.2f} m³".format(beneficio_info[4]))
    print("Transferencia Aconchos → Delicias: {:,.2f} m³".format(beneficio_info[5]))
    print("Transferencia Aconchos → BConchos: {:,.2f} m³".format(beneficio_info[6]))
