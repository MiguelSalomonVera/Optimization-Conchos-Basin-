# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 2,3: BC-FLOR using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
coalicion = ["BConchos","Florido"]
eficiencia_transferencia=0.8 #considerando canales revestidos o mixtos
restricciones_por_distrito = {
    "BConchos": {"nut_area_min": 772*.85},
    "Florido": {"nut_area_min": 844*0.82, "fodder_production_min": 20000, "corn_area_min": 300}}
restricciones_conjuntas={"fodder_production_min": {"BConchos": 20000, "Florido":20000}}
#Fodder crops 
fodder_crops_por_distrito = {
    "BConchos":["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
    "Florido": ["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"]} 
...
for prop_bc, prop_Flor in escenarios_coalicion:
    clave = (round(prop_bc, 2), round(prop_Flor, 2))
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_bc*100:.0f}% y {prop_Flor*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_bc if d == "BConchos" else prop_Flor, 2)]for d in coalicion for c in data[d]})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "BConchos":
           ajuste = (eficiencia_transferencia * step4.transfiere_Flor_a_BC) - step4.transfiere_BC_a_Flor*.8
        elif d == "Florido":
           ajuste = (eficiencia_transferencia * step4.transfiere_BC_a_Flor) - step4.transfiere_Flor_a_BC*.8
        else:
           ajuste = 0
...
# Área mínima de maíz para Delicias
    def area_maiz_rule(step4):
        min_maiz = restricciones_por_distrito["Florido"].get("corn_area_min", None)
        if min_maiz is None:
            return Constraint.Skip
        return step4.area_decision['Florido', 'MaizForrajero'] >= min_maiz
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        transferencia_opt_bc = value(step4.transfiere_BC_a_Flor)
        transferencia_opt_Flor = value(step4.transfiere_Flor_a_BC)
        sombra = {d: step4.dual.get(step4.volumen_con_transferencia[d], 0) for d in coalicion}

        resultados_coalicion[clave] = areas
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_bc,transferencia_opt_Flor)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario BC={clave[0]*100:.0f}%, FLORIDO={clave[1]*100:.0f}%")
    print("Beneficio total conjunto: ${:,.2f}".format(beneficio_info[0]))
    print("Transferencia BC → Flor: {:,.2f} m³".format(beneficio_info[1]))
    print("Transferencia Flor → BC: {:,.2f} m³".format(beneficio_info[2]))
