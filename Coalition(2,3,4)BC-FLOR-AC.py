# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,2,4: BC-Florido-AC using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
distritos = ["BConchos","Florido","Aconchos"]
models = {}
...
coalicion = ["BConchos","Florido","Aconchos"]
...
restricciones_conjuntas={"fodder_production_min": {"BConchos": 20000, "Florido":20000,"Aconchos":100 }}
#Fodder crops 
fodder_crops_por_distrito = {
    "BConchos": ["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
    "Florido": ["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"],
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["BConchos"]  # o "Florido" o "AC", son iguales
escenarios_coalicion = [(d, d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_bc, prop_FLOR, prop_AC in escenarios_coalicion:
    clave = (round(prop_bc, 2), round(prop_FLOR, 2), round(prop_AC, 2) )
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_bc*100:.0f}% , {prop_FLOR*100:.0f}% y {prop_AC*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_bc if d == "BConchos" else prop_FLOR if d == "Florido" else prop_AC, 2)]for d in coalicion for c in data[d]})
    ...
    step4.volumen_disponible_base = Param(step4.distritos, initialize={"BConchos": region["BConchos"]["volumen_mm3"] * 1e3 * prop_bc,"Florido": region["Florido"]["volumen_mm3"] * 1e3 * prop_FLOR,
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "BConchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_FLOR_a_bc)+(eficiencia_transferencia* step4.transfiere_AC_a_bc)) - ((step4.transfiere_bc_a_FLOR*.8)+(step4.transfiere_bc_a_AC*.8))
        elif d == "Florido":
           ajuste = ((eficiencia_transferencia * step4.transfiere_bc_a_FLOR)+(eficiencia_transferencia* step4.transfiere_AC_a_FLOR)) - ((step4.transfiere_FLOR_a_bc*.8)+(step4.transfiere_FLOR_a_AC*.8))
        elif d == "Aconchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_bc_a_AC)+(eficiencia_transferencia* step4.transfiere_FLOR_a_AC)) - ((step4.transfiere_AC_a_bc*.8)+(step4.transfiere_AC_a_FLOR*.8))
        else:
           ajuste = 0
...
# Área mínima de maíz para BConchos
    def area_maiz_rule(step4):
        min_maiz = restricciones_por_distrito["BConchos"].get("corn_area_min", None)
        if min_maiz is None:
            return Constraint.Skip
        return step4.area_decision['BConchos', 'MaizForrajero'] >= min_maiz
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_bc_FLOR,transferencia_opt_bc_AC,transferencia_opt_FLOR_bc,transferencia_opt_FLOR_AC,transferencia_opt_AC_bc,transferencia_opt_AC_FLOR)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario BConchos={clave[0]*100:.0f}%, Florido={clave[1]*100:.0f}%,Aconchos={clave[2]*100:.0f}%")
    print("Beneficio total conjunto: ${:,.2f}".format(beneficio_info[0]))
    print("Transferencia BConchos → Florido: {:,.2f} m³".format(beneficio_info[1]))
    print("Transferencia BConchos → Aconchos: {:,.2f} m³".format(beneficio_info[2]))
    print("Transferencia Florido → BConchos: {:,.2f} m³".format(beneficio_info[3]))
    print("Transferencia Florido → Aconchos: {:,.2f} m³".format(beneficio_info[4]))
    print("Transferencia Aconchos → BConchos: {:,.2f} m³".format(beneficio_info[5]))
