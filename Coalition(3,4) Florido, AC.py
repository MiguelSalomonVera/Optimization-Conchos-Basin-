# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 2,3: Flor-AC using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
region = {"Florido": {"volumen_mm3": 64095.84, "lamina_reportada": 1.74},
...
restricciones_conjuntas={"fodder_production_min": {"Florido": 20000, "Aconchos":100}}
#Fodder crops 
fodder_crops_por_distrito = {
    "Florido":["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"],
    "Aconchos": ["Alfalfa"]} 
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Florido"]  # o AC, son iguales
escenarios_coalicion = [(d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_Flor, prop_AC in escenarios_coalicion:
    clave = (round(prop_Flor, 2), round(prop_AC, 2))
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_Flor*100:.0f}% y {prop_AC*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_Flor if d == "Florido" else prop_AC, 2)]for d in coalicion for c in data[d]})
    ...
    step4.volumen_disponible_base = Param(step4.distritos, initialize={"Florido": region["Florido"]["volumen_mm3"] * 1e3 * prop_Flor,"Aconchos": region["Aconchos"]["volumen_mm3"] * 1e3 * prop_AC})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Florido":
           ajuste = (eficiencia_transferencia * step4.transfiere_AC_a_Flor) - step4.transfiere_Flor_a_AC*.8
        elif d == "Aconchos":
           ajuste = (eficiencia_transferencia * step4.transfiere_Flor_a_AC) - step4.transfiere_AC_a_Flor*.8
        else:
           ajuste = 0
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        transferencia_opt_Flor = value(step4.transfiere_Flor_a_AC)
        transferencia_opt_AC = value(step4.transfiere_AC_a_Flor)
        sombra = {d: step4.dual.get(step4.volumen_con_transferencia[d], 0) for d in coalicion}

        resultados_coalicion[clave] = areas
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_Flor,transferencia_opt_AC)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Flor={clave[0]*100:.0f}%, Aconchos={clave[1]*100:.0f}%")
    print("Beneficio total conjunto: ${:,.2f}".format(beneficio_info[0]))
    print("Transferencia Flor → AC: {:,.2f} m³".format(beneficio_info[1]))
    print("Transferencia AC → Flor: {:,.2f} m³".format(beneficio_info[2]))
