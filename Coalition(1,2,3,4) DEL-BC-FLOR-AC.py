# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,2,3,4: Delicias-BC-Flor-AC using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
distritos = ["Delicias","BConchos","Florido","Aconchos"]
models = {}
...
restricciones_por_distrito = {
    "Delicias": {"nut_area_min": 13830*.75, "fodder_production_min": 500000, "corn_area_min": 2800},
    "BConchos": {"nut_area_min": 772*0.85, "fodder_production_min": 20000},
    "Florido": {"nut_area_min": 844*0.82, "fodder_production_min": 20000, "corn_area_min": 300},
    "Aconchos": {"nut_area_min": 7634*.55,"fodder_production_min": 100}}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "BConchos": ["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
    "Florido": ["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"],
    "Aconchos": ["Alfalfa"]} 
...
coalicion = ["Delicias","BConchos","Florido","Aconchos"]
eficiencia_transferencia=0.8 #considerando canales revestidos o mixtos
restricciones_por_distrito = {
    "Delicias": {"nut_area_min": 13830*.75,"corn_area_min": 2800},
    "BConchos": {"nut_area_min": 772*0.85},
    "Florido": {"nut_area_min": 844*0.82,"corn_area_min": 300},
    "Aconchos": {"nut_area_min": 7634*.55}}
restricciones_conjuntas={"fodder_production_min": {"Delicias": 500000, "BConchos":20000,"Florido":20000,"Aconchos":100}}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "BConchos": ["Avena Forrajera", "Rye Grass", "Sorgo", "Alfalfa"],
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]  # o "BConchos" o "Florido" o "AC", son iguales
escenarios_coalicion = [(d, d, d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_bc, prop_Flor, prop_AC in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_bc, 2), round(prop_Flor, 2), round(prop_AC, 2))
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% , {prop_bc*100:.0f}% y {prop_Flor*100:.0f}% y {prop_AC*100:.0f}%")
#Model 
...
# Variables de decisión y Transferencia 
    ...
    step4.transfiere_del_a_bc = Var(domain=NonNegativeReals)
    step4.transfiere_del_a_Flor = Var(domain=NonNegativeReals)
    step4.transfiere_del_a_AC = Var(domain=NonNegativeReals)
    step4.transfiere_bc_a_del = Var(domain=NonNegativeReals)
    step4.transfiere_bc_a_Flor = Var(domain=NonNegativeReals)
    step4.transfiere_bc_a_AC = Var(domain=NonNegativeReals)
    step4.transfiere_Flor_a_del = Var(domain=NonNegativeReals)
    step4.transfiere_Flor_a_bc = Var(domain=NonNegativeReals)
    step4.transfiere_Flor_a_AC = Var(domain=NonNegativeReals)
    step4.transfiere_AC_a_del = Var(domain=NonNegativeReals)
    step4.transfiere_AC_a_bc = Var(domain=NonNegativeReals)
    step4.transfiere_AC_a_Flor = Var(domain=NonNegativeReals)
# Parámetros
    step4.precio_venta = Param(step4.cultivos, initialize={(d, c): data[d][c]['price'] for d in coalicion for c in data[d]})
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_bc if d == "BConchos" else prop_Flor if d == "Florido" else prop_AC, 2)]for d in coalicion for c in data[d]})
    ...
    step4.volumen_disponible_base = Param(step4.distritos, initialize={"Delicias": region["Delicias"]["volumen_mm3"] * 1e3 * prop_del,"BConchos": region["BConchos"]["volumen_mm3"] * 1e3 * prop_bc,
                                          "Florido": region["Florido"]["volumen_mm3"] * 1e3 * prop_Flor,"Aconchos": region["Aconchos"]["volumen_mm3"] * 1e3 * prop_AC })
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = ((eficiencia_transferencia * step4.transfiere_bc_a_del)+(eficiencia_transferencia* step4.transfiere_Flor_a_del)+(eficiencia_transferencia* step4.transfiere_AC_a_del)) - ((step4.transfiere_del_a_bc*.8)+(step4.transfiere_del_a_Flor*.8)+(step4.transfiere_del_a_AC*.8))
        elif d == "BConchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_bc)+(eficiencia_transferencia* step4.transfiere_Flor_a_bc)+(eficiencia_transferencia* step4.transfiere_AC_a_bc)) - ((step4.transfiere_bc_a_del*.8)+(step4.transfiere_bc_a_Flor*.8)+(step4.transfiere_bc_a_AC*.8))
        elif d == "Florido":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_Flor)+(eficiencia_transferencia* step4.transfiere_bc_a_Flor)+(eficiencia_transferencia* step4.transfiere_AC_a_Flor)) - ((step4.transfiere_Flor_a_del*.8)+(step4.transfiere_Flor_a_bc*.8)+(step4.transfiere_Flor_a_AC*.8))
        elif d == "Aconchos":
           ajuste = ((eficiencia_transferencia * step4.transfiere_del_a_AC)+(eficiencia_transferencia* step4.transfiere_bc_a_AC)+(eficiencia_transferencia* step4.transfiere_Flor_a_AC)) - ((step4.transfiere_AC_a_del*.8)+(step4.transfiere_AC_a_bc*.8)+(step4.transfiere_AC_a_Flor*.8))
        else:
           ajuste = 0
...
# Área mínima de maíz para Delicias
    def area_maiz_rule(step4):
        min_maiz = restricciones_por_distrito["Delicias"].get("corn_area_min", None)
        if min_maiz is None:
            return Constraint.Skip
        return step4.area_decision['Delicias', 'MaizForrajero'] >= min_maiz
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        transferencia_opt_del_bc = value(step4.transfiere_del_a_bc)
        transferencia_opt_del_Flor = value(step4.transfiere_del_a_Flor)
        transferencia_opt_del_AC = value(step4.transfiere_del_a_AC)
        transferencia_opt_BC_del = value(step4.transfiere_bc_a_del)
        transferencia_opt_BC_Flor = value(step4.transfiere_bc_a_Flor)
        transferencia_opt_BC_AC = value(step4.transfiere_bc_a_AC)
        transferencia_opt_Flor_del = value(step4.transfiere_Flor_a_del)
        transferencia_opt_Flor_bc = value(step4.transfiere_Flor_a_bc)
        transferencia_opt_Flor_AC = value(step4.transfiere_Flor_a_AC)
        transferencia_opt_AC_del = value(step4.transfiere_AC_a_del)
        transferencia_opt_AC_bc = value(step4.transfiere_AC_a_bc)
        transferencia_opt_AC_Flor = value(step4.transfiere_AC_a_Flor)
        sombra = {d: step4.dual.get(step4.volumen_con_transferencia[d], 0) for d in coalicion}
        resultados_coalicion[clave] = areas
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del_bc, transferencia_opt_del_Flor,transferencia_opt_del_AC, transferencia_opt_BC_del,transferencia_opt_BC_Flor,transferencia_opt_BC_AC,transferencia_opt_Flor_del,
                                       transferencia_opt_Flor_bc,transferencia_opt_Flor_AC,transferencia_opt_AC_del,transferencia_opt_AC_bc,transferencia_opt_AC_Flor)
...
#Resultados optimización conjunta 4 elementos 
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, BConchos={clave[1]*100:.0f}%,Florido={clave[2]*100:.0f}%,Aconchos={clave[2]*100:.0f}% ")
    print("Beneficio total conjunto: ${:,.2f}".format(beneficio_info[0]))
    print("Transferencia Delicias → BConchos: {:,.2f} m³".format(beneficio_info[1]))
    print("Transferencia Delicias → Florido: {:,.2f} m³".format(beneficio_info[2]))
    print("Transferencia Delicias → Aconchos: {:,.2f} m³".format(beneficio_info[3]))
    print("Transferencia BConchos → Delicias: {:,.2f} m³".format(beneficio_info[4]))
    print("Transferencia BConchos → Florido: {:,.2f} m³".format(beneficio_info[5]))
    print("Transferencia BConchos → Aconchos: {:,.2f} m³".format(beneficio_info[6]))
    print("Transferencia Florido → Delicias: {:,.2f} m³".format(beneficio_info[7]))
    print("Transferencia Florido → BConchos: {:,.2f} m³".format(beneficio_info[8]))
    print("Transferencia Florido → Aconchos: {:,.2f} m³".format(beneficio_info[9]))
    print("Transferencia Aconchos → Delicias: {:,.2f} m³".format(beneficio_info[10]))
    print("Transferencia Aconchos → BConchos: {:,.2f} m³".format(beneficio_info[11]))
    print("Transferencia Aconchos → Florido: {:,.2f} m³".format(beneficio_info[12]))
