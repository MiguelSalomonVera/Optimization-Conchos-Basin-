# PMP CODE 
#Scenario 2 and 3  interregional water market  code for Coalition 1,3: Delicias FLorido using Evapotranspiration (FAO EQ.56 AND IRRIGATION METHODS, yield fluctuation)
#%% Library import
import os
import numpy as np
...
coalicion = ["Delicias","Florido"]
eficiencia_transferencia=0.8 #considerando canales revestidos o mixtos
restricciones_por_distrito = {
    "Delicias": {"nut_area_min": 13830*.75,"corn_area_min": 2800},
    "Florido": {"nut_area_min": 844*.82,"corn_area_min": 300}}
restricciones_conjuntas={"fodder_production_min": {"Delicias": 500000, "Florido":20000}}
#Fodder crops 
fodder_crops_por_distrito = {
    "Delicias": ["MaizForrajero", "Alfalfa"],
    "Florido": ["Avena Forrajera", "MaizForrajero", "Sorgo", "Alfalfa"]} 
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]  # o Floriod, son iguales
escenarios_coalicion = [(d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_Flor in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_Flor, 2))
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% y {prop_Flor*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_Flor, 2)]for d in coalicion for c in data[d]})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = (eficiencia_transferencia * step4.transfiere_Flor_a_del) - step4.transfiere_del_a_Flor*.8
        elif d == "Florido":
           ajuste = (eficiencia_transferencia * step4.transfiere_del_a_Flor) - step4.transfiere_Flor_a_del*.8
        else:
           ajuste = 0
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        transferencia_opt_del = value(step4.transfiere_del_a_Flor)
        transferencia_opt_Flor = value(step4.transfiere_Flor_a_del)
        sombra = {d: step4.dual.get(step4.volumen_con_transferencia[d], 0) for d in coalicion}

        resultados_coalicion[clave] = areas
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del,transferencia_opt_Flor)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, Florido={clave[1]*100:.0f}%")
    ...
    print("Transferencia Florido → Delicias: {:,.2f} m³".format(beneficio_info[2]))
