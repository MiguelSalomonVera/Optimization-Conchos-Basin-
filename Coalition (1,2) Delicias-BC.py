...
region = {"Delicias": {"volumen_mm3": 976309.62, "lamina_reportada": 1.44},
          "BConchos": {"volumen_mm3": 79206.52, "lamina_reportada": 2.46}} # en m 
...
#Escenarios de disponibilidad 
niveles_disp = scenarios_disponibilidad["Delicias"]  # o "BConchos", son iguales
escenarios_coalicion = [(d, d) for d in niveles_disp]
model_coalicion = {}
resultados_coalicion = {}
beneficios_coalicion = {}
precios_sombra_coalicion = {}
for prop_del, prop_bc in escenarios_coalicion:
    clave = (round(prop_del, 2), round(prop_bc, 2))
    print(f"\n📘 Resolviendo coalición con transferencias, disponibilidades {prop_del*100:.0f}% y {prop_bc*100:.0f}%")
#Model 
...
# Parámetros
    ...
    step4.yield_ajustado = Param(step4.cultivos, initialize={(d, c): rendimientos_ajustados[d][c][round(prop_del if d == "Delicias" else prop_bc, 2)]for d in coalicion for c in data[d]})
...
#Restriccion Volumen
    def volumen_rule(step4, d):
        ...
        if d == "Delicias":
           ajuste = (eficiencia_transferencia * step4.transfiere_bc_a_del) - step4.transfiere_del_a_bc*.8
        elif d == "BConchos":
           ajuste = (eficiencia_transferencia * step4.transfiere_del_a_bc) - step4.transfiere_bc_a_del*.8
        else:
           ajuste = 0
...
#Produccion de forrajes 
    margen_relajacion = 0.10  # 20% de relajación en la producción mínima
...
#Solucion
    ...
    if resultado.solver.status == SolverStatus.ok and resultado.solver.termination_condition == TerminationCondition.optimal:
        ...
        transferencia_opt_del = value(step4.transfiere_del_a_bc)
        transferencia_opt_BC = value(step4.transfiere_bc_a_del)
        sombra = {d: step4.dual.get(step4.volumen_con_transferencia[d], 0) for d in coalicion}

        resultados_coalicion[clave] = areas
        beneficios_coalicion[clave] = (beneficio, transferencia_opt_del,transferencia_opt_BC)
...
for clave in sorted(resultados_coalicion.keys()):
    ...
    if areas is None:
        ...
        continue
    print(f"\n✅ Escenario Delicias={clave[0]*100:.0f}%, BConchos={clave[1]*100:.0f}%")
    ...
    print("Transferencia BConchos → Delicias: {:,.2f} m³".format(beneficio_info[2]))
